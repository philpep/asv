...
class Results(object):
    ...
    def _merge_results(self, old):
        # Merge existing results with old results
        # When benchmark is parameterized and new result is either failed or
        # skipped for a given param combination then keep old result.
        ...
        for key, val in six.iteritems(old_results):
            if key not in new_results:
                ...
                for idx, param_set in enumerate(itertools.product(
                        *self._benchmark_params[key])):
                    if (new_results[key][idx] is None and
